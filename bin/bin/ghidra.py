...
def run(filename):
    ...
    if r:
        ...
        click.secho(
            click.style(f"./{os.path.basename(filename)}", bold=True), fg="green"
        )
        os.system(
            f'{GHIDRA_PATH}support/analyzeHeadless {out_dir} "{proj_name}" -import "{filename}" > /dev/null 2>&1'
        )
        os.system(
            f'{GHIDRA_PATH}ghidraRun "{os.path.join(out_dir, os.path.basename(proj_file))}" > /dev/null 2>&1'
        )
